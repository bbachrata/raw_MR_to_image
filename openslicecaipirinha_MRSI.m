...
% 
%   Output:
% -     kspace_water          Separated k-space of water    (size: [#coils, nx, ny, nSlice_ACS, nTime]) (nTime = 1 for MRI)
% -     kspace_fat            Separated k-space of fat         
%              
%
...
if(~exist('InData','var'))
    error('I need Cachaca ( = data which should be processed (InData)) for preparing CAIPIRINHA! Aborting . . .')
    return
end

if(~exist('ACS_or_weights','var'))
    error('I need Glasses ( = Auto Calibration Signal (ACS)) for preparing CAIPIRINHA! Aborting . . .')
    return
end
...
if(iscell(ACS_or_weights))
    ...
    elseif(exist('AliasedSlices','var'))
        nSlice_ACS = length(AliasedSlices);
    else
        error('I need either FoV-Shifts or AliasedSlices! Aborting . . .')
        return
    end    
    ...
    if(nChannel_ACS~=nChannel)
        error('Error! The number of coils has to be the same for both inputs! Aborting . . .')
        OutData = InData;
        weights = 0;
        return;
    end
...
if(nSlice == nSlice_ACS)
    error('Nothing to do, since the InData has the same amount of slices as the ACS data . . .')
    OutData = InData;
    weights = 0;
    return;
end
...
clearvars -except quiet_flag OutData is2D weights


%% 4. Separate water and fat in "2D k-space"
kspace_water = OutData(:,:,:,1:size(OutData,4)/2);
kspace_fat = OutData(:,:,:,(size(OutData,4)/2+1):end);
